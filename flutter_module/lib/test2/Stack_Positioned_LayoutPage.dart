...
width和height用于指定需要定位元素的宽度和高度。注意，Positioned的width、height
和其它地方的意义稍微有点区别，此处用于配合left、top 、right、 bottom来定位组件，
举个例子，在水平方向时，你只能指定left、right、width三个属性中的两个，
如指定left和width后，right会自动算出(left+width)，如果同时指定三个属性则会报错，
垂直方向同理。

 */
class Stack_Positioned_LayoutPage extends StatefulWidget {
  @override
  _Stack_Positioned_LayoutPageState createState() =>
      _Stack_Positioned_LayoutPageState();
}

class _Stack_Positioned_LayoutPageState
    extends State<Stack_Positioned_LayoutPage> {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          title: Text("Stack_Positioned_LayoutPage"),
        ),
        ...
        body: ConstrainedBox(
          ...
          child: Stack(
            ...
            children: <Widget>[
              // 没有使用Positioned定位 则使用alignment对齐方式
              Container(
                child:
              ...
              Positioned(
                // 垂直方向没有指定 则垂直方向使用alignment对齐方式
                left: 18.0,
                child: Text("I am Jack"),
              ),
              Positioned(
                // 水平方向没有指定 则水平方向使用alignment对齐方式
                top: 18.0,
